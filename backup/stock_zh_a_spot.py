...
async def fetch_all_pages_async(url: str, base_params: Dict, proxy=None) -> List[Dict]:
    """异步获取所有页面数据"""
    # 首先获取总数以计算页数
    ...
    if proxy is not None:
        ...
        proxy_auth = proxy_auth = aiohttp.BasicAuth(proxy['auth']['key'], proxy['auth']['pwd'])
    async with aiohttp.ClientSession(proxy=server, proxy_auth=proxy_auth) as session:
...
async def stock_zh_a_spot_em_async(proxy=None) -> pd.DataFrame:
    """
    ...
    results = await fetch_all_pages_async(url, params, proxy=proxy)
